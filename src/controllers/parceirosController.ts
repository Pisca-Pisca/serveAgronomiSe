...
export default {
  async createParceiro(req: any, res: any) {
    try {
      ...
      const imageRef = ref(storageRef, "Parceiros/" + fileName);
      ...
      await uploadTask.on("state_changed",
        ...
        () => {
         ...
         getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
            addDoc(collection(db, "Parceiros"), { 
              ...body,
              logoParceiro: downloadURL
            });
          });
        }
      );
      ...
      return res.status(400).json({
        data: {},
        error: "Ocorreu um erro ao criar o Parceiro.",
        success: false,
      });
    }
  },
  ...
  async findAllParceiro(req: any, res: any) {
    try {
     const parceirosRef = await getDocs(collection(db, "Parceiros"));

     const parceiros = parceirosRef.docs.map((doc: any) => ({ id: doc.id, ...doc.data() }));

      return res.status(200).json(parceiros);
    } catch (error) {
      return res.status(400).json({
        data: [],
        error: "Ocorreu um erro ao buscar os Parceiro.",
        success: false,
      });
    }
  },
